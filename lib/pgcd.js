...
writeUnprocessedDocs = function(docs, callback) {
	if (docs) {
		log.info('Writing unprocessed docs to file for later reference.');

		WriteCSV.writeCSVArrayToFile(docs, 'unprocessedDocs.csv', log, function(err, result) {
			if (err) {
				log.error(err);
				log4js.shutdown(function() { process.exit(8) });
			}
		...
		callback(null, 'placeholder for writeUnprocessedDocs');
	}
}
